...
void main()
{
  ...
  //Calculate toon diffuse intensity
  ...
  for (int index = 0; index < numberOfLights; index++) 
  {
    ...
    float diffuseToonIntensity = max(0.0, dot(normalDirection, lightDirection));
    float diffuseToonLevel = floor(diffuseToonIntensity * TOON_LEVELS);
    diffuseToonIntensity = diffuseToonLevel / TOON_LEVELS;

    vec3 diffuseReflection = attenuation
      * vec3(light.diffuse) * vec3(material.diffuse)
      * diffuseToonIntensity;
    ...
    {
      float specularToonIntensity = pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), max(0.00001, material.shininess));
      float toonLevelSpecular = floor(specularToonIntensity * TOON_LEVELS);
      specularToonIntensity = toonLevelSpecular / TOON_LEVELS;

      specularReflection = attenuation * vec3(light.specular) * vec3(material.specular)
        * specularToonIntensity;
    }
    totalLighting = totalLighting + diffuseReflection + specularReflection;
  }

  vec4 diffuseTex = vec4(0.0, 1.0, 0.0, 1.0);
